...
BOTH backends support GLM-5.1 *and* GLM-5.2 (DSA cross-layer index sharing), full or LoRA:
  ...
  * ``"glm-native"`` -- the vendored *fused* TileLang kernels (``SparseMLA`` + ``lighting_indexer`` under
    ...
    served by sglang; glm-native only matches sglang numerically on the *train* side.

Both backends are LoRA-capable. The DSA indexer (``wq_b`` / ``wk`` / ``weights_proj``) is excluded
from LoRA by default in the miles launcher. On the glm-native backend the indexer adapters get no
