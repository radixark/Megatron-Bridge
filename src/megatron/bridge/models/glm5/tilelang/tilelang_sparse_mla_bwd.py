...
@tilelang.jit(
    ...
    pass_configs={
        ...
        # Aggressive smem merge can alias buffers that are still live and
        # produce NaN dQ/dKV, so keep it disabled for correctness.
    ...
    def sparse_mla_bwd_kernel(
        ...
        with T.Kernel(S, B, kv_group * NH, threads=threads) as (s_i, by, bz):
            ...
            for i_i in T.Pipelined(NS, num_stages=num_stages):
                ...
                # A padded slot contains -1. Clamp its address and load a true
                # zero instead of allowing 0 * garbage/inf to become NaN.
                for bi_i in T.Parallel(BS):
                ...
                for s in range(split_store):
                    ...
                    # Padded slots have exact-zero P/dP columns, so the clamped
                    # address turns these atomics into no-ops instead of OOB writes.
