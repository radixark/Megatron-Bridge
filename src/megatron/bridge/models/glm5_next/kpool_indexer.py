...
"""kpool-compressed lightning indexer for GLM-5.3-Flash (``glm5_next``) DSA layers.

GLM-5.3 scores the indexer against *pooled* keys: every ``kpool`` consecutive keys of a
sequence are collapsed into one key with a learned softmax gate (``index_kpool_compress_gate``)
plus an additive position embedding (``index_kpool_compress_ape``). The top-k is selected over
pools and expanded back to token indices; the current token's incomplete tail pool is always
selected (``index_kpool_always_select_tail``).

All index arithmetic is per packed sequence: pools are counted per sequence
(:func:`pool_boundaries`) and a query only sees pools in ``[pool_base, pool_base +
eligible_pools)`` of its own sequence, matching what the serving engine computes one request
at a time. Ported from ``radixark/miles`` (``miles_plugins/models/glm5_next/ops/kpool_indexer.py``);
the scoring kernel is the vendored TileLang ``indexer_fwd_interface``.
"""
...
def _indexer_replay_manager():
    """miles' R3 indexer-replay manager, when miles is importable (optional hook)."""
    try:
        from miles.utils.replay_base import indexer_replay_manager
    except ImportError:
        return None
    return indexer_replay_manager


def pool_boundaries(cu_seqlens: torch.Tensor, kpool: int) -> torch.Tensor:
    """Cumulative pool counts per packed sequence (``len // kpool`` complete pools each)."""
...
def build_pooled_keys(
    ...
    """Softmax-gated pooling of ``kpool`` consecutive indexer keys, per packed sequence."""
...
def append_tail_and_pad(
    ...
    """Append the query's incomplete tail pool to a token top-k and pad to ``pad_multiple``."""
...
def kpool_select_topk(
    ...
    """Score pooled keys, pick the top pools per query and expand to ``[t, 1, width]`` token ids.

    Padded slots are ``-1`` (ignored by ``SparseMLA``). When miles' indexer replay is enabled the
    pool top-k comes from the recorded rollout selection instead of the live scores.
    """
    ...
    if pooled_k.shape[0] > 0:
        ...
        # packed-sequence helpers (pool_boundaries, ...) stay importable without it.
    ...
    replay = _indexer_replay_manager()
    if replay is not None and replay.enabled:
        topk_fn = replay.get_topk_fn(
...
__all__ = ["SPARSE_MLA_BLOCK", "append_tail_and_pad", "build_pooled_keys", "kpool_select_topk", "pool_boundaries"]
