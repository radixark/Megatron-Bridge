...
class SlimeMLASelfAttention(MLASelfAttention):
    ...
    def _slime_forward(
        ...
        *,
        ...
        assert packed_seq_params is not None and packed_seq_params.qkv_format == "thd", (
            "The slime DSA backend (dsa_attention_backend='slime') requires the thd layout "
            ...
            "Use --qkv-format thd, or select the default 'megatron-bridge' backend."
        )
    ...
    def _slime_topk(
        ...
        :meth:`_slime_index_qkw`, so an adapter on them is *used* and differentiable. BUT the fused
        ...
        index_q, index_k, weights = self._slime_index_qkw(
        ...
        # ACTIVATION RECOMPUTE: the slime path is thd-only (asserted in _slime_forward), so the
        ...
        # here because slime never runs bshd. The rest of _slime_forward is functionally pure
    ...
    def _slime_index_qkw(self, indexer, hidden_states, q_compressed, packed_seq_params):
