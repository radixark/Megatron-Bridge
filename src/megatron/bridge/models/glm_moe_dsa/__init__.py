...
BOTH backends support GLM-5.1 *and* GLM-5.2 (DSA cross-layer index sharing), full or LoRA:
  ...
  * ``"megatron-bridge"`` (default) -- the portable *unfused* megatron-core DSA kernels
  ...
  * ``"slime"`` -- the vendored *fused* TileLang kernels (``SparseMLA`` + ``lighting_indexer`` under
    ``fused/``, driven by ``SlimeMLASelfAttention`` in ``slime_mla.py``). Matches slime's rollout
    ...
    served by sglang; slime only matches sglang numerically on the *train* side.

Both backends are LoRA-capable. The DSA indexer (``wq_b`` / ``wk`` / ``weights_proj``) is excluded
from LoRA by default in the miles launcher. On the slime backend the indexer adapters get no
